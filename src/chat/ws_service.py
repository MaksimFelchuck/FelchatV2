...
class ChatWebSocketService:
    ...
    async def send_personal_message(
        ...
        try:
            ...
            self._logger.error(f"Error sending message from {from_user_id} to {to_user_id}: {e}")
            return False
    ...
    def _is_message_blocked(self, from_user_id: int, to_user_id: int) -> bool:
        ...
        if self.is_blocked(from_user_id, to_user_id):
            self._logger.warning(f"Message blocked: {from_user_id} -> {to_user_id} (users are blocked)")
            return True
        return False
    ...
    async def _broadcast_message(self, message_data: str, to_user_id: int, from_user_id: int) -> None:
    ...
    async def _send_to_user_sessions(self, message_data: str, user_id: int, user_type: str) -> None:
    ...
    async def get_history(self, user1_id: int, user2_id: int, limit: int = None) -> list[dict]:
        """
